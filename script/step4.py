...
# Distinct colors for each gene (これがZ最大時の色 = 元の濃い色)
gene_colors = {
...
def create_figure(selected_gene=None):
    ...
    for i, idx in enumerate(disp_idx):
        ...
        # Z低 (norm_z≈0): 薄めだが濃く調整 (lighten_factor=0.7 → 元の70%白寄せ)
        # Z高 (norm_z≈1): 元の濃い色 (lighten_factor=0.0 → base_colorそのまま)
        norm_z = z_peak / NUM_SAMPLES
        ...
        fig.add_trace(go.Scatter3d(
            ...
            marker=dict(
                size=MARKER_SIZE, 
                color=marker_color,
                line=dict(color='black', width=1.5)  # 黒輪郭追加
            ),
            hovertemplate=(
                f'<b>{gene}</b><br>'
    ...
    # 0% white background (anti-overlap) - inside the circle
    fig.add_trace(go.Scatter3d(
