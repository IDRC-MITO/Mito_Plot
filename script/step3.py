...
def plot_maf_circular_3d_compact_z(tsv_file, output_html="Mitochondrial_MAF_Circular3D_CompactZ.html"):
    ...
    for i, sample in enumerate(samples):
        maf_values = data[sample].apply(clean_and_convert).fillna(0)
        ...
        hover_texts = [
            f"Sample: {sample}<br>Position: {pos}<br>MAF: {maf*100:.2f}%"
            for pos, maf in zip(positions, maf_values)
        ]
    ...
    for gene, start, end in genes:
        ...
        thetas_band = np.linspace(start_angle, end_angle, 100)
    ...
    fig.update_layout(
        ...
        showlegend=False,
        title="3D Circular MAF Plot (Compressed Z-axis)",
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python step3.py <maf_tsv_file>")
        sys.exit(1)

    tsv_file = sys.argv[1]
    plot_maf_circular_3d_compact_z(tsv_file)
