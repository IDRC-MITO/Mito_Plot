...
def lighten_color(hex_color, factor):
    """Lighten a hex color by the given factor (0.0 dark -> 1.0 bright)."""
    rgb = np.array(mcolors.to_rgb(hex_color))
    light_rgb = np.clip(rgb + (1 - rgb) * factor, 0, 1)
    return mcolors.to_hex(light_rgb)

if len(sys.argv) != 2:
    print("Usage: python mtdna_viewer.py <maf_tsv_file>")
    sys.exit(1)
...
def safe_maf_convert(x):
    """Safely convert MAF values to fractions (0.0-1.0)."""
    if pd.isna(x):
        return 0.0
    x_str = str(x).strip()
    if "%" in x_str:
        try:
            return float(x_str.replace("%", "")) / 100
        except:
            return 0.0
    try:
        val = float(x_str)
...
maf_matrix = data.iloc[:, sample_start:].map(safe_maf_convert).fillna(0).values
...
threshold = 0.01
variant_count = (maf_matrix > threshold).sum(axis=1)
max_maf = np.max(maf_matrix, axis=1)
r_scale = np.clip(max_maf / np.max(max_maf) if np.max(max_maf) > 0 else max_maf, 0.0, 1.0)
...
def create_figure(selected_gene=None):
    ...
    for i, idx in enumerate(disp_idx):
        ...
        r_pos = ZERO_RADIUS + (r_sc * (1.10 - ZERO_RADIUS))

        base_color = gene_colors.get(gene, "#CCCCCC")
        norm_z = z_peak / NUM_SAMPLES if NUM_SAMPLES > 0 else 0
        lighten_factor = 0.7 * (1 - norm_z)
        marker_color = lighten_color(base_color, lighten_factor)
        ...
        fig.add_trace(go.Scatter3d(
            ...
            hovertemplate=(
                ...
                f"<b>Max MAF: {max_maf[idx]:.1%}</b><br>"
    ...
    fig.add_trace(go.Scatter3d(
        x=(ZERO_RADIUS * 0.8) * np.cos(full_thetas),
        y=(ZERO_RADIUS * 0.8) * np.sin(full_thetas),
        z=np.zeros(n_ring),
        mode="lines",
    ...
    for gname, s, e in genes:
        ...
        r_label = 1.22
        fig.add_trace(go.Scatter3d(
            x=[r_label * np.cos(t_mid)],
            y=[r_label * np.sin(t_mid)],
            z=[0.25],
            mode="text",
            text=[gname],
    ...
    fig.update_layout(
        title=dict(
            text=f"3D mtDNA MAF Viewer ({tsv_file})",
            x=0.5,
            y=0.95,
...
app.layout = html.Div([
    html.Div([
        ...
        html.Div(
            f"File: {tsv_file} | Sites >1%: {total_sites} | Samples: {NUM_SAMPLES}",
