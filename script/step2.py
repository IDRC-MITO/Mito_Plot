...
def plot_maf(tsv_file, output_html="Mitochondrial_MAF.html"):
    ...
    for sample in samples:
        ...
        maf_values = data[sample].apply(clean_and_convert)
        thetas = positions / MT_LENGTH * 360

        hover_texts = [
            f"Sample: {sample}<br>Pos: {pos}<br>MAF: {maf*100:.2f}%"
    ...
    for gene, start, end in genes:
        ...
        fig.add_trace(go.Scatterpolar(
            ...
            name=gene,
            hoverinfo="text",
            text=[f"{gene}: {start}-{end}"] * len(thetas_band),
            showlegend=False
        ))
    ...
    fig.update_layout(
        ...
        showlegend=False,
        title="Mitochondrial MAF and Gene Locations (Interactive)"
    )
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python step2.py <maf_tsv_file>")
        sys.exit(1)
